...
export default function CatalogClient({ products, categories, brands, kashrut, dietary, settings }: Props) {
  ...
  useEffect(() => { setVisibleCount(12); }, [selected, selectedBrands, selectedTags]);
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product: Product) => {
      const categoryMatch = selected === "בחירה" || product.category?.includes(selected);
      const brandMatch    = selectedBrands.length === 0 || selectedBrands.includes(product.brand);
      const kashrutMatch  = selectedKashrut.length === 0 || selectedKashrut.some((k) => product.kashrut?.includes(k));
      const dietaryMatch  = selectedDietary.length === 0 || selectedDietary.some((d) => product.dietary?.includes(d));
      const lc = searchTerm.toLowerCase().trim();
      const searchMatch   = lc === "" || product.title?.toLowerCase().includes(lc) || product.info?.toLowerCase().includes(lc) || product.sku?.toLowerCase().includes(lc);
  ...
  const sortedProducts = useMemo(() => {
    ...
    switch (selectedSort) {
      case "תאריך (מחדש לישן)": return arr.sort((a, b) => new Date(b.date_created).getTime() - new Date(a.date_created).getTime());
      case "תאריך (מישן לחדש)": return arr.sort((a, b) => new Date(a.date_created).getTime() - new Date(b.date_created).getTime());
      case "מחיר (מזול ליקר)":  return arr.sort((a, b) => a.price - b.price);
      case "מחיר (מיקר לזול)":  return arr.sort((a, b) => b.price - a.price);
      case "מותג":               return arr.sort((a, b) => a.brand.localeCompare(b.brand, "he"));
      case "שם (א-ת)":           return arr.sort((a, b) => a.title.localeCompare(b.title, "he"));
      default:                   return arr;
    }
  }, [filteredProducts, selectedSort]);

  const displayedProducts = useMemo(() => sortedProducts.slice(0, visibleCount), [sortedProducts, visibleCount]);

  // ── Infinite scroll ───────────────────────────────────────────────────────────
  const totalRef = React.useRef(0);
  totalRef.current = sortedProducts.length;
    ...
    const handleScroll = () => {
      ...
      if (window.scrollY + window.innerHeight >= document.documentElement.scrollHeight - 300) {
        setVisibleCount((prev) => {
          ...
          return Math.min(prev + 12, totalRef.current);
        });
      }
    };
  ...
  const faqItems = useMemo(() => [
    ...
    { title: "רכיבים", content: selectedProduct?.components_ads },
    {
      title: "מכיל",
      content: (
        <div className="flex items-start gap-4 justify-between">
          <div>{selectedProduct?.containing_ads}</div>
          <img src="contains-icon.svg" alt="" className="w-25 h-25 object-contain flex-shrink-0" />
        </div>
      ),
    },
    {
  ...
  return (
    <main dir="rtl" className="min-h-screen bg-[#F5CA5F]">
      ...
      <section className="sticky top-0 z-50 bg-[#ffffff] px-[1px] pt-0 pb-[2px]">
          ...
          <div className="mx-auto flex max-w-[1600px] lg:flex-row lg:items-center lg:justify-between lg:gap-6">
            ...
            <div className="hidden lg:flex lg:flex-1 lg:items-center lg:justify-center lg:gap-5">
              ...
              <div className="relative">
                <button onClick={() => setOpen(!open)}
                  ...
                  <span className="whitespace-nowrap text-[20px] font-normal text-[#D41A68] transition-colors duration-300 group-hover:text-[#ffffff]">בחירה</span>
                  ...
                  <div className="absolute right-0 top-[72px] z-50 w-full lg:w-[980px] rounded-[32px] bg-[#46BAB9] p-8 px-5 shadow-2xl backdrop-blur-sm">
                    ...
                    <div className="mb-5 flex items-center gap-4">
                      ...
                      <div className="flex flex-wrap gap-3">
                        {kashrut.map((item) => (
                          <button key={item.id} onClick={() => toggleFilter(item.name, selectedKashrut, setSelectedKashrut)}
                            className={`cursor-pointer rounded-[12px] border border-[#D41A68] px-2 py-2 leading-[16px] text-[24px] font-medium transition-all duration-200 ${selectedKashrut.includes(item.name) ? "bg-[#F5CA5F] text-black" : "bg-white text-black hover:bg-[#D41A68] hover:text-white"}`}>
                            {item.name}
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="mb-5 flex items-center gap-4">
                      ...
                      <div className="flex flex-wrap gap-3">
                        {brands.map((item) => (
                          <button key={item.id} onClick={() => toggleFilter(item.name, selectedBrands, setSelectedBrands)}
                            className={`cursor-pointer rounded-[12px] border border-[#D41A68] px-2 py-2 leading-[16px] text-[20px] font-medium transition-all duration-200 ${selectedBrands.includes(item.name) ? "bg-[#F5CA5F] text-black" : "bg-white text-black hover:bg-[#D41A68] hover:text-white"}`}>
                            {item.name}
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="mb-5 flex items-center gap-4">
                      ...
                      <div className="flex flex-wrap gap-3">
                        {dietary.map((item) => (
                          <button key={item.id} onClick={() => toggleFilter(item.name, selectedDietary, setSelectedDietary)}
                            className={`cursor-pointer rounded-[12px] border border-[#D41A68] px-2 py-2 leading-[16px] text-[24px] font-medium transition-all duration-200 ${selectedDietary.includes(item.name) ? "bg-[#F5CA5F] text-black" : "bg-white text-black hover:bg-[#D41A68] hover:text-white"}`}>
                            {item.name}
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => { setSelectedTags([]); setSelectedBrands([]); setSelectedDietary([]); setSelectedKashrut([]); }}
              ...
              <div className="flex h-[54px] w-full max-w-full lg:max-w-[533px] items-center rounded-full border border-[#D41367] bg-white px-6 shadow-sm">
                <input type="text" placeholder="חפש מוצר" value={searchTerm}
            ...
            <div className="flex justify-center mb-[10px] lg:block">
              <div className="w-[150px] lg:w-[220px] h-[72px] flex justify-end items-end lg:justify-end lg:items-center">
                <a href="/">
                  <Image src="/leftsidelogo.webp" alt="left logo" width={440} height={140} priority
        ...
        {/* ── CATEGORY BAR ── */}
        <section style={{ backgroundColor: settings?.category_nav_bg_color }}
          className="relative z-2 px-6 py-[17px] rounded-b-[6px] bg-[#D41367]">
          <div className="mx-auto max-w-[1360px]">
            <Swiper modules={[Autoplay]} spaceBetween={18} loop={false} speed={800}
              ...
              {sortedCategories.map((item, index) => {
                ...
                return (
                  <SwiperSlide key={index}>
                    <button onClick={() => setSelected(selected === item.title ? "בחירה" : item.title)}
                      className={`w-full rounded-xl px-2 py-3 lg:px-3 lg:py-2 text-center shadow-sm transition-all duration-200 cursor-pointer ${isActive ? "bg-[#46BAB9]" : "bg-[#f5f5f5] hover:bg-[#46BAB9]"}`}>
                      <h3 className={`text-[12px] lg:text-[17px] leading-[16px] font-bold transition-colors duration-200 ${isActive ? "text-white" : "text-black hover:text-white"}`}>
                        {item.title}
                      </h3>
                      <div className={`mx-auto mt-1 flex h-[19px] w-[35px] items-center justify-center rounded-full text-[13px] leading-[16px] font-bold transition-all duration-200 ${isActive ? "bg-white text-[#46BAB9]" : "bg-[#D41A68] text-white hover:bg-white hover:text-[#D41A68]"}`}>
                        {item.count}
                      </div>
                    </button>
                  </SwiperSlide>
        ...
        {/* ── MOBILE SEARCH + FILTER BAR (shown only on mobile, below categories) ── */}
        <div className="lg:hidden bg-white px-3 py-3 flex items-center gap-2 border-t border-gray-100">

          {/* Filter button — right side */}
          <button onClick={() => setOpen(!open)}
            className="flex h-[44px] shrink-0 items-center gap-2 rounded-full border border-[#D41A68] bg-white px-4 shadow-sm active:bg-[#D41A68] active:text-white">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none"
              viewBox="0 0 24 24" stroke="#D41A68" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M3 6h18M7 12h10M11 18h2" />
            </svg>
            <span className="text-[14px] font-bold text-[#D41A68]">סינון</span>
          </button>

          {/* Search input — left side */}
          <div className="flex flex-1 h-[44px] items-center rounded-full border border-[#D41367] bg-white px-4 shadow-sm">
            <input type="text" placeholder="חפש מוצר" value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-transparent text-right text-[15px] font-bold text-black outline-none placeholder:text-[#999]" />
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
              strokeWidth={2.5} stroke="currentColor" className="h-5 w-5 shrink-0 text-black">
              <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-4.35-4.35m1.85-5.15a7 7 0 1 1-14 0 7 7 0 0 1 14 0Z" />
            </svg>
          </div>
        </div>
        ...
        {/* Mobile filter panel */}
        {open && (
          <div className="lg:hidden bg-[#46BAB9] px-4 py-5 space-y-4">

            <div className="flex flex-wrap gap-2">
              {kashrut.map((item) => (
                <button key={item.id} onClick={() => toggleFilter(item.name, selectedKashrut, setSelectedKashrut)}
                  className={`rounded-[10px] border border-[#D41A68] px-3 py-2 text-[14px] font-medium transition-all duration-200 ${selectedKashrut.includes(item.name) ? "bg-[#F5CA5F] text-black" : "bg-white text-black"}`}>
                  {item.name}
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {brands.map((item) => (
                <button key={item.id} onClick={() => toggleFilter(item.name, selectedBrands, setSelectedBrands)}
                  className={`rounded-[10px] border border-[#D41A68] px-3 py-2 text-[14px] font-medium transition-all duration-200 ${selectedBrands.includes(item.name) ? "bg-[#F5CA5F] text-black" : "bg-white text-black"}`}>
                  {item.name}
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {dietary.map((item) => (
                <button key={item.id} onClick={() => toggleFilter(item.name, selectedDietary, setSelectedDietary)}
                  className={`rounded-[10px] border border-[#D41A68] px-3 py-2 text-[14px] font-medium transition-all duration-200 ${selectedDietary.includes(item.name) ? "bg-[#F5CA5F] text-black" : "bg-white text-black"}`}>
                  {item.name}
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between pt-2">
              <button onClick={() => setOpen(false)}
                className="rounded-full bg-white px-5 py-2 text-[14px] font-bold text-[#D41A68]">
                סגור
              </button>
              <button onClick={() => { setSelectedBrands([]); setSelectedDietary([]); setSelectedKashrut([]); }}
                className="rounded-full bg-[#D41A68] px-5 py-2 text-[14px] font-black text-white">
                נקה סינון
              </button>
            </div>
          </div>
      ...
      <section className="px-10 py-12 pt-[16px] bg-[#F5CA5F]"
        ...
        <div className="relative mx-auto max-w-[1200px] mb-[60px] flex justify-end">
          <button onClick={() => setSortOpen(!sortOpen)}
            ...
            <span className="text-[20px] font-normal leading-[17px] text-[#D41A68] transition-all duration-300 group-hover:text-white">{selectedSort}</span>
            ...
            <div className="absolute left-0 top-[0px] z-12 w-[300px] rounded-[10px] bg-[#D41A68] p-4 shadow-2xl">
              ...
              <div className="flex flex-col gap-5">
                {sortOptions.map((item) => (
                  <button key={item} onClick={() => { setSelectedSort(item); setSortOpen(false); }}
                    className={`cursor-pointer rounded-full border-[1px] px-6 py-3 text-center text-[20px] font-bold transition-all duration-200 ${selectedSort === item ? "border-[#46BAB9] bg-[#46BAB9] text-white" : "border-[#46BAB9] bg-[#F3F3F3] text-black hover:bg-[#46BAB9] hover:text-white"}`}>
                    {item}
                  </button>
                ))}
              </div>
              ...
              <div className="relative z-10 flex h-full flex-col bg-white px-[25px] pb-10 pt-8">
                <div className="flex items-start justify-end">
                  ...
                  <div className="ml-[-14px] relative">
                    <Image src={product.brand_image} alt={product.brand} width={220} height={70}
                ...
                <div className="relative flex-1">
                  <p className="whitespace-pre-line text-center text-[13px] leading-[16px] font-medium text-black">{product.info}</p>
                </div>
                <p className="mt-2 text-center text-[12px] leading-[22px] font-medium text-black">{product.tags}</p>
                <p className="mt-2 text-center text-[12px] leading-[22px] font-medium text-black">מק"ט {product.sku}</p>
                      ...
                      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between md:gap-4">
                        ...
                        <div className="shrink-0">
                          <Suspense fallback={<div className="h-[60px] w-[100px] bg-gray-100 animate-pulse" />}>
                            <Barcode value={selectedProduct.sku} format="CODE128" width={1.2} height={60} fontSize={9} margin={0} />
                          </Suspense>
                        </div>
                      </div>
                    </div>
                      ...
                      {galleryImages.map((img: string, index: number) => {
                        ...
                        return (
                          <Image key={img} src={img} alt={`gallery-${index}`} fill
                            ...
                            priority={isActive}
                            className={`absolute inset-0 object-contain transition-all duration-700 ${isActive ? "opacity-100 scale-100" : "opacity-0 scale-95"}`} />
                        );
                      })}
                    </div>
                    <div className="mt-5 flex justify-center gap-4">
                      {galleryImages.map((_: any, index: number) => (
                        <button key={index} onClick={() => setActiveImage(index)}
                          className={`h-5 w-5 rounded-full border-2 border-[#EC1974] transition-all duration-300 ${activeImage === index ? "bg-[#EC1974] scale-110" : "bg-white"}`} />
                      ))}
                    </div>
                  </div>
                </div>
